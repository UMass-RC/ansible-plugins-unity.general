...
# https://github.com/vbotka/ansible-plugins
...
def lists_mergeby(l1, l2, index):
    ...
    if not isinstance(l1, Sequence):
        raise AnsibleFilterError(
            "First argument for lists_mergeby must be list. %s is %s" % (l1, type(l1))
        )

    if not isinstance(l2, Sequence):
        raise AnsibleFilterError(
            "Second argument for lists_mergeby must be list. %s is %s" % (l2, type(l2))
        )

    if not isinstance(index, string_types):
        raise AnsibleFilterError(
            "Third argument for lists_mergeby must be string. %s is %s" % (index, type(index))
        )
    ...
    for l in (l1, l2):
        for elem in l:
            if not isinstance(elem, Mapping):
                raise AnsibleFilterError(
                    "Elements of list arguments for lists_mergeby must be dictionaries. Found {0!r}.".format(
                        elem
                    )
                )
...
def any2items(x, key="key", override=False):
    ...
    if not isinstance(key, string_types):
        raise AnsibleFilterError(
            "Argument key for any2items must be string. %s is %s" % (key, type(key))
        )
    if not isinstance(override, bool):
        raise AnsibleFilterError(
            "Argument override for any2items must be boollean. %s is %s"
...
def items2dict2(mylist, key_name="key", value_name="value", default_value=None):
    ...
    if not isinstance(mylist, Sequence):
        raise AnsibleFilterError(
            "First argument for community.general.items2dict2 requires a list. %s is %s"
            % (mylist, type(mylist))
        )
